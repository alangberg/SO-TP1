...
int SchedPSJF::tick(int cpu, const enum Motivo m) {
	if (m == EXIT) {
		if (ready_tasks.empty()) {
			...
			ready_tasks.pop();
			// se pone el flag en 0 por las dudas, pues puede haber un tick con motivo EXIT justo después de un load
			nueva_tarea = 0;
			return next_task;
		}
	} else if (nueva_tarea == 1) {
		// si m == TICK y se cargó una nueva tarea 
